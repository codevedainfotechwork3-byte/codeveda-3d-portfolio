      ...
      <div className="max-w-7xl mx-auto">
        <Reveal variant="blur">
          ...
          <h1 className="font-display text-5xl md:text-7xl font-semibold tracking-tighter mt-3 max-w-4xl">
            Selected <span className="italic text-gradient-animated">work</span>.
          </h1>
          ...
          {projects.map((p, i) => (
            <Reveal key={p.title} delay={i * 0.05}>
              <TiltCard intensity={5}>
                <a
                  href="#"
                  className="group block card-elevated gradient-border rounded-3xl p-1 hover:border-[var(--color-glow)]/40 transition relative overflow-hidden"
                >
                  <div className={`relative aspect-[16/10] rounded-[20px] overflow-hidden bg-gradient-to-br ${p.color}`}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-32 h-32 rounded-full bg-foreground/5 backdrop-blur-xl border border-foreground/10 flex items-center justify-center group-hover:scale-125 group-hover:rotate-12 transition duration-700">
                        <span className="font-display italic text-3xl font-bold opacity-60">{p.title.split(" ").map(w => w[0]).join("")}</span>
                      </div>
                    </div>
                    <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/5 transition duration-500" />
                    <div className="absolute top-4 right-4 w-10 h-10 rounded-full bg-background/60 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 group-hover:rotate-45 transition duration-500">
                      <ArrowUpRight className="w-4 h-4" />
                    </div>
                  </div>
                  <div className="p-6" style={{ transform: "translateZ(20px)" }}>
                    <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{p.tag}</span>
                    <h3 className="font-display text-2xl font-semibold mt-2 group-hover:italic transition">{p.title}</h3>
